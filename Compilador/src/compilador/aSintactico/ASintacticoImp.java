...
public class ASintacticoImp extends ASintactico {
	...
	private void reconoceRestoPrograma()throws Exception {
			...
			if (!hayErrorLexico(token)
					&& (comparaTokens(token.getCategoria(), EnumToken.TIPO))) {
				reconoceZonaTipos();
			}
			if (!hayErrorLexico(token)
					&& (comparaTokens(token.getCategoria(), EnumToken.TVAR))) {
				reconoceDeclaraciones();
			}
			if (!hayErrorLexico(token)
					&& (comparaTokens(token.getCategoria(), EnumToken.PROC))) {
...
//////////////////////Ojo con el dos
				...
				while (!hayErrorLexico(token)
						&& (!comparaTokens(token.getCategoria(), EnumToken.INICIO))){
					if (!hayErrorLexico(token)
							&& (compruebaTokens(token.getCategoria(), EnumToken.PROC))) {
						direccion=1;
						reconoceProcedimiento();
						epilogo(nivel+1);
						codigo="irIndice;.";
			...
			if (!hayErrorLexico(token)
				&& (compruebaTokens(token.getCategoria(), EnumToken.INICIO))) {
		    reconoceCuerpo();
		}	
	}
	...
	private ArrayList<Parametro> reconoceParametrosProcedimiento() throws Exception{
		...
		while(!fin){
			...
			token = ALexico.getInstance().obtenerToken();
			while (!hayErrorLexico(token)
					&& (comparaTokens(token.getCategoria(), EnumToken.PYCOMA))
					&& !hayError) {
				token = ALexico.getInstance().obtenerToken();
				if (!hayErrorLexico(token)
						&& (compruebaTokens(token.getCategoria(),
			...
			if (!hayErrorLexico(token)
				...
				if (!hayErrorLexico(token)
					...
					if (!existeIDNivel(nombre)) {
						...
						token = ALexico.getInstance().obtenerToken();
						String tipo="";
						if (comparaTokens(token.getCategoria(),EnumToken.DIGITO)) tipo="integer";
	...
	private ArrayList obtenerArrayConstante(String tipo) throws Exception {
		...
		if (tipo.equals("integer")){
			...
			while (!hayErrorLexico(token)&&(comparaTokens(token.getCategoria(),EnumToken.COMA))){
				...
				else if (!hayErrorLexico(token)&& (compruebaTokens(token.getCategoria(),EnumToken.ID))&&(tipoConstante(token.getLexema()).equals("integer"))){
					aConst.add(token.getLexema());
					token = ALexico.getInstance().obtenerToken();
				}
			}
		...
		if (tipo.equals("boolean")){
			...
			while (!hayErrorLexico(token)&&(comparaTokens(token.getCategoria(),EnumToken.COMA))){
				...
				else if (!hayErrorLexico(token)&& (compruebaTokens(token.getCategoria(),EnumToken.ID))&&(tipoConstante(token.getLexema()).equals("boolean"))){
					aConst.add(token.getLexema());
					token = ALexico.getInstance().obtenerToken();
				}
			}
	...
	private void reconoceRestoDeclaracion() throws Exception {
		while (!hayErrorLexico(token)
				&& (comparaTokens(token.getCategoria(), EnumToken.ID))
				&& !hayError) {
			reconoceDeclaracion();
		}
	}	
				...
				while (!comparaTokens(token.getCategoria(),EnumToken.PC)){
						...
						if (!hayErrorLexico(token)&&comparaTokens(token.getCategoria(),EnumToken.ID)){
							...
							if (!hayErrorLexico(token)&&comparaTokens(token.getCategoria(),EnumToken.CA)){
								...
								else if (!hayErrorLexico(token)&&(comparaTokens(token.getCategoria(),EnumToken.COMA)||comparaTokens(token.getCategoria(),EnumToken.PC))){
									PropTipos param=((PropTiposPro)t.dameTipo(asignada,nivel)).getParametros().get(i).getTipo();
