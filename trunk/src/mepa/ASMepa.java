...
/**
 * Realiza el analisis sintactico de las sentencias de un programa MEPa. A la
 * vez, realiza la carga de las instrucciones y resuelve las direcciones para
 * las etiquetas del programa
 * 
 * @since jdk 1.6
 * @see mepa.ALMepa
...
public class ASMepa implements Testeable {
	...
	/**
	 * Tabla que mantiene la asociacion de etiquetas y direcciones <string,int>
	 * o la asociacion entre etiquetas aun no declaradas y un vector de
	 * instrucciones que la referencian <String, list>.
	 */
	private Hashtable<String, Object> etiquetas = new Hashtable<String, Object>();

	/**
	 * Tabla que mantiene las etiquetas aun no declaradas Permite chequear,
	 * luego del analisis sintactico, si no quedan etiquetas que fueron
	 * referenciadas pero no declaradas
	 */
	private Hashtable<String, Integer> etiquetasNoDeclaradas = new Hashtable<String, Integer>();

	/**
	 * Mantiene las instrucciones que va cargando a medida que progresa el
	 * analisis sintactico
	 */
	...
	/**
	 * Convierte un string a entero capturando posibles errores de numeros mal
	 * formados
	 * 
	...
	/**
	 * Ingresa la declaracion de una eiqueta. Dispara error si la etiqueta ya
	 * fue declarada previamente. Modifica la instrucciones anteriores que la
	 * referencian (si hay alguna) y la elimina de la tabla de etiquetas no
	 * referenciadas
	 */
	...
	private void instruccion() throws Exception {
		...
		case TokenMepa.APCT:
			...
			TokenMepa t = token;
			int op1 = numero();
			instrucciones.add(new Instruccion(t.codigo, t.numLinea, t.lexema,
					op1));
			break;
		case TokenMepa.SUMA:
		case TokenMepa.SUST:
		...
		case TokenMepa.DUPT:
			instrucciones.add(new Instruccion(token.codigo, token.numLinea,
					token.lexema));
			break;
		// saltos
		case TokenMepa.DSVS:
		...
		case TokenMepa.IMLV:
			...
			break;
		default:
			throw new Exception("instruccion no reconocida!, nombre="
					+ token.lexema + " codigo=" + token.codigo);
		}
	...
	private void leer1Parametro() throws Exception {
		TokenMepa t = token;
		nextToken();
		if (token.codigo != TokenMepa.NUMERO)
			throw new MepaException(15, t.numLinea);
	 ...
	 * 
	 * @param b1
	 *            si es true el primer parametro puede ser un numero negativo
	 * @param b2
	 *            si es true el segundo parametro puede ser un numero negativo
	 */
	private void leer2Parametros(boolean b1, boolean b2) throws Exception {
		...
		nextToken();
		if (token.codigo != TokenMepa.COMA)
			throw new MepaException(17, token.numLinea);

		int op2 = 0;
		if (!b2) {
			nextToken();
			if (token.codigo != TokenMepa.NUMERO)
				throw new MepaException(18, token.numLinea);
	...
	private void leer3Parametros() throws Exception {
		...
		if (token.codigo != TokenMepa.COMA)
			throw new MepaException(17, token.numLinea);
		/*
		 * nextToken(); if(token.codigo!=TokenMepa.NUMERO) throw new
		 * MepaException(18,token.numLinea); int op2 =
		 * convertirAEntero(token.lexema);
		 */
		int op2 = numero();
		nextToken();
		if (token.codigo != TokenMepa.COMA)
			throw new MepaException(19, token.numLinea);
		nextToken();
		if (token.codigo != TokenMepa.NUMERO)
			throw new MepaException(20, token.numLinea);
	...
	private void leerEtiqueta() throws Exception {
		...
		if (token.codigo != TokenMepa.ETIQ)
			throw new MepaException(16, t.numLinea);
		Instruccion inst = new Instruccion(t.codigo, t.numLinea, t.lexema,
				token.lexema);
	...
	/**
	 * <listaSentencias>::= <etiqueta> <restoListaSentencia> | <instruccion>
	 * <listaSentencias> | PARA
	 */
	...
	/**
	 * <numero> ::= NUMERO | - NUMERO atributo sintetizado int
	 */
	...
	/**
	 * @param inst
	 *            la instruccion que hace ref a la etiqueta
	 * @param line
	 *            nro de linea de la instruccion
	 */
	...
	public void run() throws Exception {
		if (debug)
			System.out.println("[MEPA] Cargando instrucciones...");
		nextToken();
		// el programa debe empezar con INPP
		if (token.codigo != TokenMepa.INPP)
			throw new MepaException(7, token.numLinea);
		...
		if (token.codigo != TokenMepa.EOF)
			throw new MepaException(8, token.numLinea);

		// Chequeo que todas las etiquetas referenciadas estan declaradas
		if (!etiquetasNoDeclaradas.isEmpty())
			throw new MepaException(9);

		if (debug)
			System.out.println("[MEPA] Analisis y carga....OK");
	}
