...
/**
 * Mepa.java Maquina Virtual del lenguaje
 * 
 * @since jdk1.6
 */
...
public class Mepa implements Testeable {
	...
	/**
	 * Constructor Carga los parametros de configuracion
	 */
	public Mepa() {
	...
	private void ejecutar(Instruccion instruccion) throws Exception {
		...
		try {
			...
			case TokenMepa.APVL:
				...
				s++;
				M[s] = M[D[instruccion.op1] + instruccion.op2];
				break;
			case TokenMepa.ALVL:
				M[D[instruccion.op1] + instruccion.op2] = M[s];
				s--;
				break;
			...
			case TokenMepa.APDR:
				s++;
				M[s] = D[instruccion.op1] + instruccion.op2;
				break;
			case TokenMepa.APVI:
				s++;
				M[s] = M[M[D[instruccion.op1] + instruccion.op2]];
				break;
			case TokenMepa.ALVI:
				M[M[D[instruccion.op1] + instruccion.op2]] = M[s];
				s--;
				break;
			// Arreglos
			case TokenMepa.APAR:
			...
			case TokenMepa.PUAR:
				for (int k = 1; k <= instruccion.op3; k++) {
					s++;
					M[s] = M[D[instruccion.op1] + instruccion.op2 + k - 1];
				}
				break;
			case TokenMepa.POAR:
			...
			case TokenMepa.CONT:
				if (M[s] < instruccion.op1 || M[s] > instruccion.op2)
					throw new MepaException(0, "->" + instruccion.nombre + " "
							+ instruccion.op1 + "," + instruccion.op2
							+ " valor en pila =" + M[s], instruccion.numLinea);
				break;
			// Division por cero
			case TokenMepa.DIVC:
				if (M[s] == 0)
					throw new MepaException(1, "->" + instruccion.nombre,
							instruccion.numLinea);
				break;
			...
			throw new MepaException(27, "->" + instruccion.nombre,
					instruccion.numLinea);
		}
	}// end ejecutar
	...
	private void leer(Instruccion ins) throws Exception {
		...
		if (dig > 57 || dig < 48)// ERROR -Si despues de leer el menos no viene
									// un digito
	...
	/**
	 * Metodo principal Toma una instruccion y la ejecuta
	 * 
	 * @throws java.lang.Exception
	...
	public void run() throws Exception {
		...
		try {
			...
			if (debug)
				System.out.println("[MEPA] Ejecutando...");
	...
	public void setSourceFile(String file) throws IOException {
		/*
		 * if(asint!=null){ asint.finish(); asint = null; }
		 */
